...
bool AP_Compass_HMC5843_SPI::read_register(uint8_t reg, uint8_t *value)
{
    uint8_t tx[2];
    uint8_t rx[2];
    tx[0] = reg | 0b10000000; // read flag
    tx[1] = 0;
...
bool AP_Compass_HMC5843_SPI::write_register(uint8_t reg, uint8_t value)
{
    uint8_t tx[2];
    tx[0] = reg;
    tx[1] = value;
...
// Read Sensor data
bool AP_Compass_HMC5843_SPI::read_raw()
{
    uint8_t buff[7];
    uint8_t tx[7];
    memset(&tx, 0, 7);
    ...
    rx = (((int16_t)buff[1]) << 8) | buff[2];
    rz = (((int16_t)buff[3]) << 8) | buff[4];
    ry = (((int16_t)buff[5]) << 8) | buff[6];
...
// accumulate a reading from the magnetometer
void AP_Compass_HMC5843_SPI::accumulate(void)
{
    if (!_initialised) {
        // someone has tried to enable a compass for the first time
        // mid-flight .... we can't do that yet (especially as we won't
 ...
 *  re-initialise after a IO error
 */
bool AP_Compass_HMC5843_SPI::re_initialise()
{
    if (!write_register(ConfigRegA, _base_config) ||
        !write_register(ConfigRegB, magGain) ||
        !write_register(ModeRegister, ContinuousConversion))
...
AP_Compass_HMC5843_SPI::init()
{
    ...
    _spi = hal.spi->device(AP_HAL::SPIDevice_HMC5843);
    ...
    // SPI has to be HMC5983, which has same settings as HMC5883
    product_id = AP_COMPASS_TYPE_HMC5883L;
    calibration_gain = 0x60;
    /*
      note that the HMC5883 datasheet gives the x and y expected
...
// Read Sensor data
bool AP_Compass_HMC5843_SPI::read()
{
    if (!_initialised) {
        // someone has tried to enable a compass for the first time
        // mid-flight .... we can't do that yet (especially as we won't
